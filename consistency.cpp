...
vector<QString> consistencyCheckCorrect(vector<string> &xml_buffer)
{
    ...
    for (int i = 0; i < xml_buffer.size(); i++)   /* Iteration on strings in the buffer */
    {
        ...
        {                 /* Error message */
            ...
            error_text.append("Error in line number ");
            error_text.append(QString::number(i+1));
            error_text.append("\n"); //cout << "Error in line number " << i + 1 << "\n";
            error_text.append("Missing closing tag");
            error_text.append("\n"); //cout << "Missing closing tag" << "\n";
            error_text.append("Missing tag is : </");
            error_text.append(QString::fromStdString(tagNames.top()));
